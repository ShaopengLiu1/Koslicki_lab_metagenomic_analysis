...
class CMash_CountEstimator:
	...
	def cmash_stream_ji_vector(self, other_list, threads=8):
		Y = list()
		for obj in other_list:
			Y.append(self.stream_cmash_for_ji(obj))
		return np.array(Y)
	...
	def ground_truth_ji_vector(self, other_list, threads=8):
		Y = list()
		for obj in other_list:
			Y.append(self.ground_truth_jaccard(obj))
		return np.array(Y)
...
if __name__ == '__main__':
	...
	parser.add_argument('-l', '--out_name_label', type=str, help="Addional label for output files", default="out")
	...
	for file in query_list:
		print(file)
		CE = CMash_CountEstimator(n=max_h, ksize=ksize, input_file_name=file, rev_comp=rev_comp,
		                          threads=num_threads, full_kmer=full_kmer)
		ce_list1.append(CE)
	...
	if not skip_est:
		...
		out_name = "est_JI_matrix_k" + str(ksize) + "_" + out_label + ".csv"
